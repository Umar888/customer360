...
class _CartItemPageState extends State<CartItemPage>
  ...
  late StreamSubscription<RecommendationCartState> subscription;

  @override
  initState() {
    super.initState();
    // FirebaseAnalytics.instance.setCurrentScreen(screenName: 'CartItemPagePage');
  ...
  List<Widget> generateListOfBrowsedItems(
      LoadCartItemsSuccess state, int indexOfList) {
    if (userBrowsedWidget.isEmpty) {
      for (var i = 0;
          i < state.productCartBrowseItemsModel!.productCart.length;
          i++) {
        for (var j = 0;
            j <
                state.productCartBrowseItemsModel!.productCart[i]
          ...
          if (widget.inventorySearchBloc.state.productsInCart.isNotEmpty) {
            for (asm.Records records
              ...
              if (state.productCartBrowseItemsModel!.productCart[i]
                      .wrapperinstance.records[j].childskus.first.skuENTId ==
                  records.childskus!.first.skuENTId) {
                recommendedScreenBlocBloc.add(UpdateProductInCart(
          ...
          userBrowsedWidget.add(BlocProvider.value(
            ...
            child: BrowseItemTileWidget(
              customer: widget.customerInfoModel,
              recommendedState: "LoadCartItemsSuccess",
              mainIndex: i,
              childIndex: j,
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<RecommendationCartBloc, RecommendationCartState>(
      ...
      builder: (context, state) {
        if (state is LoadCartItemsSuccess) {
          generateListOfBrowsedItems(
              state, state.productCartBrowseItemsModel!.productCart.length);
          return Stack(
            children: [
              Column(
                children: [
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              //   child: userBrowsedWidget.isNotEmpty
                              //       ? Text(
                              //           viewalltxt,
                              //           style: textThem.headline2,
                              //         )
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              state.productCartFrequentlyBaughtItemsModel!
                                      .productCartOthers.isNotEmpty
                                  ? Text(
                                      viewalltxt,
                                      style: textThem.headline2,
                                    )
